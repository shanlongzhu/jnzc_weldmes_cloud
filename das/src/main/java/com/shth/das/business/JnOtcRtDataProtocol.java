...
public class JnOtcRtDataProtocol {
     ...
     * @return 返回一个集合对象
     */
    public List<JNRtDataUI> jnRt2DataUiAnalysis(String clientIp, String str) {
        if(CommonUtils.isNotEmpty(str) && str.length() == 288){
            str = str.toUpperCase();
            if("7E".equals(str.substring(0,2)) && "22".equals(str.substring(10,12)) && "7D".equals(str.substring(286,288))){
                List<JNRtDataDB> rtdata = new ArrayList<>();
                JNRtDataUI data = new JNRtDataUI();
                //采集编号对应的采集信息
                Map<String,TaskClaimIssue> otcTaskClaimMap = CommonMap.OTC_TASK_CLAIM_MAP;
                //采集模块编号
                data.setGatherNo(Integer.valueOf(str.substring(16,20),16).toString());
                if(otcTaskClaimMap.size() > 0 && otcTaskClaimMap.containsKey(data.getGatherNo())){
                    TaskClaimIssue taskClaimIssue = otcTaskClaimMap.get(data.getGatherNo());
                    if(taskClaimIssue != null){
                        data.setWelderId(taskClaimIssue.getWelderId());
                        data.setWelderNo(taskClaimIssue.getWelderNo());
                        data.setWelderName(taskClaimIssue.getWelderName());
                        data.setWelderDeptId(taskClaimIssue.getWelderDeptId());
                ...
                //当前系统时间 yyyy-MM-dd HH:mm:ss
                String nowDateTime = DateTimeUtils.getNowDateTime();
                for(int a = 0;a <=160;a += 80){
                    //年
                    String year = CommonUtils.hexToDecLengthJoint(str.substring(44 + a, 46 + a), 2);
                    //月
                    String month = CommonUtils.hexToDecLengthJoint(str.substring(46 + a, 48 + a), 2);
                    //日
                    String day = CommonUtils.hexToDecLengthJoint(str.substring(48 + a, 50 + a), 2);
                    //时
                    String hour = CommonUtils.hexToDecLengthJoint(str.substring(50 + a, 52 + a), 2);
                    //分
                    String minute = CommonUtils.hexToDecLengthJoint(str.substring(52 + a, 54 + a), 2);
                    //秒
                    String second = CommonUtils.hexToDecLengthJoint(str.substring(54 + a, 56 + a), 2);
                    String strDate = year + "-" + month + "-" + day + " " + hour + ":" + minute + ":" + second;


                    //实际电流
                    data.setElectricity(BigDecimal.valueOf(Integer.valueOf(str.substring(56+a,60+a),16))
                            .divide(new BigDecimal("10"),1,BigDecimal.ROUND_HALF_UP));
                    //实际电压
                    data.setVoltage(BigDecimal.valueOf(Integer.valueOf(str.substring(60 + a, 64 + a), 16))
                            .divide(new BigDecimal("10"), 1, BigDecimal.ROUND_HALF_UP));
                    //送丝速度
                    data.setWireFeedRate(BigDecimal.valueOf(Integer.valueOf(str.substring(64 + a, 68 + a), 16))
                            .divide(new BigDecimal("10"), 1, BigDecimal.ROUND_HALF_UP));
                    //给定电流
                    data.setPresetEle(BigDecimal.valueOf(Integer.valueOf(str.substring(68 + a, 72 + a), 16))
                            .divide(new BigDecimal("10"),1,BigDecimal.ROUND_HALF_UP));
                    //给定电压
                    data.setPresetVol(BigDecimal.valueOf(Integer.valueOf(str.substring(72 + a, 76 + a), 16))
                            .divide(new BigDecimal("10"),1,BigDecimal.ROUND_HALF_UP));
                }
