...
public class NettyDecoder extends ByteToMessageDecoder {
    ...
    NettyDecoder() {
        //第一次验证
        sxMap.put("4E455430", 21);
        //第二次验证
        sxMap.put("4C4A5348", 128);
        //其他
        sxMap.put("FE5AA5", 0);
    }
    ...
    private void otcRecursionReadBytes(ChannelHandlerContext ctx, ByteBuf message, List<Object> out) {
        ...
        if (bufNum < 2) {
            tempMsg.writeBytes(message.readBytes(bufNum));
        } else {
            ...
            if ("7E".equals(header)) {
                ...
                if (length > 0 && bufNum >= length) {
                    ...
                    Map<String, Object> map = JnRtDataProtocol.jnRtDataDecoderManage(ctx, str);
                    if (map.size() > 0) {
                        out.add(map);
                    }
    ...
    private void sxRecursionReadBytes(ChannelHandlerContext ctx, ByteBuf message, List<Object> out) {
        ...
        if (bufNum <= 5) {
            ...
            String header = CommonUtils.bytesToHexString(headBytes);
            //判断是否为两次握手验证
            if (sxMap.containsKey(header)) {
                //获得数据包长度
                Integer length = sxMap.get(header);
                //可读字节是否多于数据包长度（是否为完整包）
                if (length > 0 && bufNum >= length) {
                    byte[] bytes = new byte[length];
                    message.readBytes(bytes);
                    String str = CommonUtils.bytesToHexString(bytes);
                    Map<String, Object> map = SxRtDataProtocol.sxRtDataDecoderManage(ctx, str);
                    if (map.size() > 0) {
                        out.add(map);
                    }
